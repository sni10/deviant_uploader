...
- **Отслеживание в БД**: База данных SQLite/PostgreSQL для отслеживания загрузок и галерей
...
### Веб-интерфейсы
- **Stats Dashboard**: Веб-дашборд для просмотра текущей статистики девиаций, дневных снэпшотов и истории вотчеров с автосинхронизацией всех галерей
- **Charts Dashboard**: Интерактивные графики и визуализация статистики с фильтрацией по девиациям и временным периодам
- **Upload Admin Interface**: Полноценный веб-интерфейс для пакетного управления загрузками с поддержкой drag-and-drop

### Продвинутые возможности
- **Upload Presets**: Система сохраняемых пресетов для быстрой настройки параметров загрузки (теги, галереи, mature-флаги и т.д.)
- **Batch Operations**: Пакетная загрузка, публикация и удаление девиаций через веб-интерфейс
- **Rate Limiting**: Автоматическая обработка rate limits DeviantArt API с экспоненциальным backoff
- **Database Abstraction**: Поддержка SQLite и PostgreSQL через единый интерфейс
- **Responsive UI**: Адаптивный интерфейс с Bootstrap 5 для работы на любых устройствах
...
### Веб-интерфейс DeviantArt Manager

Приложение предоставляет три веб-интерфейса для управления вашим контентом на DeviantArt.

**Запуск веб-сервера:**
1. Убедитесь, что выполнена базовая настройка (`.env`, `python fetch_user.py`, `python fetch_galleries.py`)
...
Доступны три интерфейса с адаптивным навбаром для переключения между ними.

#### Stats Dashboard (`http://localhost:5000/`)

Дашборд для просмотра и синхронизации статистики ваших девиаций.

**Возможности:**
- Просмотр текущих просмотров, избранного и комментариев по всем девиациям
- Дневные дельты (прирост/падение) по каждой метрике
- Миниатюры, заголовки, даты публикации, mature-флаги
- Отображение количества вотчеров пользователя и изменения за последний день
- **Автоматическая синхронизация всех галерей** - кнопка Sync перебирает все галереи с интервалом 3 секунды
- Сортировка по любому столбцу (просмотры, избранное, комментарии, дата публикации)
- Комбинированный Score (views + favourites × 10) для быстрой оценки популярности

**API endpoints:**
- `GET /api/stats` — текущая статистика с дневными дельтами
- `POST /api/stats/sync` — синхронизация статистики для галереи
- `GET /api/options` — список пользователей и галерей
- `GET /api/user_stats/latest?username=...` — последний снэпшот вотчеров
...
**Возможности:**
- Визуализация агрегированной статистики (просмотры, избранное, комментарии) за выбранный период
- Фильтрация по конкретным девиациям (выбор из списка с миниатюрами)
- Гибкие временные периоды (7, 14, 30 дней)
- История вотчеров пользователя с графиками изменений
- Интерактивные графики на Chart.js с возможностью масштабирования

**API endpoints:**
- `GET /api/charts/deviations` — список всех девиаций для фильтрации
- `GET /api/charts/aggregated?period=7&deviation_ids=...` — агрегированная статистика
- `GET /api/charts/user-watchers?username=...&period=7` — история вотчеров

#### Upload Admin Interface (`http://localhost:5000/upload_admin.html`)

Полноценный веб-интерфейс для пакетного управления загрузками на DeviantArt.

**Возможности:**
- Сканирование папки `upload/` и отображение файлов с миниатюрами
- **Upload Presets** - создание и управление пресетами с настройками (теги, галереи, mature-флаги, artist comments)
- Применение пресетов к выбранным девиациям одним кликом
- **Batch Operations:**
  - Stash - пакетная загрузка файлов в DeviantArt Stash
  - Publish - пакетная публикация stash-элементов
  - Upload - комбинированная операция (stash + publish)
  - Delete - удаление файлов и записей из БД
- Фильтрация по статусу (new, stashed, published)
- Отображение статусов загрузки с иконками и цветами
- Responsive design для работы на планшетах и десктопах

**API endpoints:**
- `POST /api/admin/scan` — сканирование папки upload
- `GET /api/admin/drafts` — получение всех девиаций из БД
- `GET /api/admin/galleries` — список галерей
- `GET /api/admin/presets` — список пресетов
- `POST /api/admin/presets` — сохранение пресета
- `POST /api/admin/apply-preset` — применение пресета к девиациям
- `POST /api/admin/stash` — пакетная загрузка в stash
- `POST /api/admin/publish` — пакетная публикация
- `POST /api/admin/upload` — комбинированная загрузка (stash+publish)
- `POST /api/admin/delete` — удаление файлов и записей
- `GET /api/admin/thumbnail/<id>` — миниатюра девиации

**Rate Limiting:**
Все операции синхронизации учитывают rate limits DeviantArt API. При получении ответа `429 user_api_threshold` сервис выполняет несколько попыток с экспоненциальным backoff и безопасно останавливает текущий прогон, не блокируя токен.
